...
const PersonalityStep = ({ data, onUpdate, onNext, onPrevious, selectedTags, setSelectedTags }: PersonalityStepProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (tagsError) {
          console.error('PersonalityStep: Error fetching tags:', tagsError);
        } else {
          console.log('PersonalityStep: Fetched tags:', tagsData?.length || 0);
          setAllTags(tagsData || []);
        }
      } catch (error) {
        console.error('PersonalityStep: Error in fetchData:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Calculate available tags (tags not already selected)
  ...
  // Form validation
  const isValid = corePersonality.length >= 50;

  // Update parent component when personality fields change
  useEffect(() => {
    onUpdate({
      personality: {
        core_personality: corePersonality,
  ...
  }, [corePersonality, knowledgeBase, scenarioDefinition, onUpdate]);

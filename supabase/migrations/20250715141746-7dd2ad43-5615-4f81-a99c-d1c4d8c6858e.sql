...
-- Step 2: Log the update for debugging
INSERT INTO public.profiles (id, username) 
SELECT auth.uid(), 'migration_log_' || extract(epoch from now())::text
WHERE NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid())
ON CONFLICT (id) DO NOTHING;

-- Step 3: Clean up any context entries that might reference the old field name
...
-- Step 4: Ensure we only have the correct context_type for time and weather
